...
import { View, Text, TouchableWithoutFeedback, Image, Dimensions, StyleSheet, StatusBar, Platform } from 'react-native';
...
export default class Header extends Component {
    ...
    static propTypes = {
        ...
        leftImgStyle: PropTypes.oneOfType([        // HeaderLeft样式
            PropTypes.number,
            PropTypes.object
        ]),
                    ...
                    {!isTabBar ?
                        (leftImageHttpUrl ?
                            /^http(s*)/.test(leftImageHttpUrl) ?
                                ...
                                <Image source={require('../images/sucang-l.png')} style={[Styles.leftImageStyle, leftImgStyle]} /> :
                            leftText ?
    ...
    renderStatusBar() {
        return (
            <StatusBar translucent={true} backgroundColor={ Colors.transparent} barStyle={this.props.barStyle} />
        )
    }

    render() {
        return (
            <View style={[{ height: Metrics.HEADER_HEIGHT, width: Metrics.screenWidth, backgroundColor: this.props.backgroundColor, flexDirection: 'row', justifyContent: 'center', alignItems: 'center' }]}>
...
const Styles = StyleSheet.create({
    container: { flex: 1, paddingTop: Metrics.STATUSBAR_HEIGHT, height: Metrics.HEADER_HEIGHT, width: Metrics.screenWidth, backgroundColor: Colors.titleMain, flexDirection: 'row', justifyContent: 'center', alignItems: 'center' },
    ...
    leftStyle: { fontSize: 16, color: Colors.main, fontWeight: 'normal' },
    ...
    leftImageStyle: { borderColor: Colors.main },
});
